...
class _SignupScreenState extends State<SignupScreen> {
                  ...
                  child: Padding(
                    ...
                    Consumer<AuthController>(builder: (context, authController, _) {
                      return DropdownButtonHideUnderline(child: DropdownButton2<String>(
                        ...
                        value: authController.selectedPname,
                        items: authController.subscriptionPakageList.map((item) {
                        ...
                        onChanged: (value) {
                          if (value != null) {
                            final selectedItem = authController.subscriptionPakageList.firstWhere(
                                  (element) => element.pname == value,
                            );

                            authController.selectPackage(value, selectedItem.id ?? 0);
                            debugPrint("Selected ID: ${selectedItem.id}");
                          }
                        },
                        ...
                        menuItemStyleData: const MenuItemStyleData(
                          height: 48,
                          overlayColor: MaterialStatePropertyAll(Colors.grey), // On hover/press
                        ),
                      ));
                    }),
  ...
  validate(AuthController controller) async {
    ...
    final data = UserModel(
      ...
      pakageId: controller.selectedId.toString(),
    );

    controller.registerUser(data, (value) async{
...
class UserModel {
  ...
  UserModel({this.userName, this.phone, this.email, this.password,this.pakageId});
}
